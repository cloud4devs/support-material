...
YAML
  depends_on = [
    kubectl_manifest.cluster_autoscaler_service_account,
  ]
}
...
YAML
  depends_on = [
    kubectl_manifest.cluster_autoscaler_service_account,
    kubectl_manifest.cluster_autoscaler_cluster_role,
  ]
}
...
YAML
  depends_on = [
    kubectl_manifest.cluster_autoscaler_service_account,
    kubectl_manifest.cluster_autoscaler_cluster_role,
    kubectl_manifest.cluster_autoscaler_role,
  ]
}
      ...
      containers:
        - image: k8s.gcr.io/autoscaling/cluster-autoscaler:v1.22.3
          ...
          command:
            ...
            - --node-group-auto-discovery=asg:tag=k8s.io/cluster-autoscaler/enabled,k8s.io/cluster-autoscaler/${aws_eks_cluster.this.name}
